...
def _prism_numeric_id(seq_id: str) -> str:
    """'CLIKC1111122222333334' → '1111122222333334' (숫자만 추출)"""
    return re.sub(r"[^0-9]", "", seq_id)
 
 
def _prism_rprt_type(file_name: str) -> str:
    """파일명에 '보고서' 포함 여부로 RPRT_TYPE 결정: B or A"""
...
def build_prism_save_path(req: "PrismRequest", year: str, seq_id: str, seq: int, ext: str) -> str:
    """ 저장 경로: /{file_dir}/{type}/{year}/{numeric_id}{ext} (예: /attachment/prism/2024/1111122222333334_attach_1.pdf) """
    numeric_id = _prism_numeric_id(seq_id)
    path = os.path.join("/", req.file_dir, req.type, year,
                        f"{numeric_id}{ext}")
    os.makedirs(os.path.dirname(path), exist_ok=True)
    return path
...
class BillListCrawler:
    ...
    async def get_total_pages(page, end_btn_selector=None):
        ...
        try:
            ...
            # fallback: 페이지 내 모든 a/button 순회 → 최댓값
            mx = 1
...
async def _extract_bill_attachments(page, view_class, base_url, req, bi_cn, year, items=None):
    detail_url = page.url
    if not items: return {}
    # bill: BI_FILE_NM / policy: ORG_FILE_NM 둘 다 허용
        ...
        for idx in range(len(titles)):
            ...
            try:
                ...
                await page.wait_for_function(
                    f"() => window.location.pathname !== '{urlparse(current_url).path}'",
                    timeout=5000
                )
                new_url = page.url
...
async def _go_to_prism_page(page, page_no: int, timeout: int = 20000) -> bool:
    """
    PRISM 페이지 이동: div.page-links 의 숫자 텍스트 클릭.
    현재 표시 범위(1~10)에 없으면 다음 버튼으로 구간 이동 후 클릭.
    """
    try:
        # 현재 표시된 페이지 링크에서 숫자 찾기
        links = await page.query_selector_all("div.page-links a.page-link")
        for link in links:
            ...
            if txt == str(page_no):
                ...
                await page.wait_for_selector(
                    "table.tstyle_list tbody tr", timeout=timeout)
                await page.wait_for_timeout(300)
                return True
 
        # 현재 범위에 없으면 다음(▶) 버튼 클릭으로 구간 이동
...
async def _extract_policy_attachments(page, view_class, base_url, req, outbbs_cn, year, items=None):
    ...
    # ── 방법 1: DOWNURL regex로 URL 추출 → 해당 링크 클릭 ────────
    if file_url_item and file_url_item.regex:
        detail_html = await page.content()
        if view_class:
            try:
        ...
        for seq, raw_url in enumerate(raw_urls, start=1):
            ...
            try:
                if is_js_call:
                    # onclick 값에 해당 문자열이 포함된 링크 탐색
                    # raw_url_clean 에서 고유값(숫자 인자) 추출해 셀렉터 구성
                    args = re.findall(r"[\'\"]?(\d+)[\'\"]?", raw_url_clean)
                    link_sel = None
                    for arg in reversed(args):  # 뒤쪽 인자일수록 고유값
                        candidate = f"a[onclick*='{arg}']"
                        try:
                            if await page.locator(candidate).count() == 1:
                                link_sel = candidate; break
                        except Exception: continue
                    if not link_sel and args:
                        link_sel = f"a[onclick*='{args[-1]}']"
                    if not link_sel:
                ...
                if await link.count() == 0:
                    raise ValueError(f"링크 없음: {link_sel}")
    ...
    # ── 방법 2: view_class 내 a 태그 순회 클릭 fallback ──────────
    root = page
    if view_class:
        try:
            el = await page.query_selector(normalize_selector(view_class))
    ...
    for el in await root.query_selector_all("a"):
        ...
        try:
            ...
            async def _do_fallback_download():
                async with page.expect_download(timeout=10000) as dl_info:
                    await el.click()
                dl = await dl_info.value
            ...
            try:
                seq += 1
                resolved_name, save_path, dl_url = await asyncio.wait_for(_do_fallback_download(), timeout=10.0)
                file_size = os.path.getsize(save_path)
                if file_size > MAX_FILE_SIZE_BYTES:
                    os.remove(save_path); save_path = ""
                    print(f"[*] [POLICY fallback] 크기 초과 ({file_size/1024/1024:.1f}MB) → 삭제: {resolved_name}", flush=True)
                else:
                    save_path = save_path.replace("\\", "/")
...
async def _extract_prism_attachments(
    page,
    ...
    for li in await page.query_selector_all("div.file_list li"):
        ...
        # ── 비공개 여부 판단: li 다음 형제 p.fs15 에 공개예정 문구 확인 ──
        # li 부모(ul) → 다음 형제 p 탐색
        is_private = False
        try:
            ...
            if "이후 공개 예정" in notice or "비공개" in notice:
                is_private = True
        except Exception:
            pass
        ...
        if is_private:
            ...
            seq += 1
            attachments.append({
                "org_file_nm": hint_name,
                "sys_file_nm": "",
                "file_seq":    str(seq),
                "rprt_type":   _prism_rprt_type(hint_name),
            })
            continue
        ...
        try:
            ...
            seq          += 1
            resolved_name = hint_name or f"file_{seq}"
            rprt_type     = _prism_rprt_type(resolved_name)
            try:
                await page.goto(detail_url, wait_until="domcontentloaded", timeout=30000)
                await page.wait_for_timeout(300)
        ...
        attachments.append({
            "org_file_nm": resolved_name,
            "sys_file_nm": save_path,
            "file_seq":    str(seq),
            "rprt_type":   rprt_type,
        })
        ...
        try:
            for idx, list_item in enumerate(list_data):
                ...
                try:
                    ...
                    img_urls = re.findall(r'<img[^>]+src="([^"]+)"', detail_html, re.IGNORECASE)
                    img_urls = [u for u in img_urls if not any(k in u for k in [
                        "/resource/", "/koglOpen/", "btn_preview", "pdf.png",
                        "synap", "opentype", "/images/common/", "/images/sub/",
                        "/images/board/", "ico_pdf", "ico_jpg", "ico_hwp", "ico_", "/tiny/"])]
                    if img_urls:
                        img_results = []
                        ...
                        if img_results:
                            ...
                            detail["ORG_FILE_NM"] = one_or_json(nms)
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            try:
                ...
                if raw_down and re.fullmatch(r'\d+', raw_down.strip()):
                    full_page_html = await page.content()
                    url_m = re.search(r'["\']([^"\']*pAtFileSn[^"\']*)["\']',
                                    full_page_html, re.IGNORECASE)
                    if url_m:
                        resolved_url = re.sub(r'pAtFileSn=[^&"\']+',
                                            f'pAtFileSn={raw_down.strip()}', url_m.group(1))
                        resolved_url = re.sub(r'pRptNo=[^&"\']+', f'pRptNo={vid}', resolved_url)
                        detail["DOWNURL"] = urljoin(base_url, resolved_url)
                elif raw_down and re.fullmatch(r'\d+', raw_down.strip()):
                    full_page_html = await page.content()
                    url_m = re.search(r'["\']([^"\']*pAtFileSn[^"\']*)["\']',
                                      full_page_html, re.IGNORECASE)
                    if url_m:
                ...
                if img_urls:
                    ...
                    for img_seq, img_url in enumerate(img_urls, start=1):
                        ...
                        _, ext = os.path.splitext(img_param or urlparse(full_img_url).path)
                        if not ext: ext = ".png"
                        img_path = build_policy_save_path(
                            req, year, outbbs_cn, img_seq, ext).replace("_attach_", "_img_")
                        try:
                            async with httpx.AsyncClient(
                                    headers={"User-Agent": USER_AGENT},
                                    timeout=httpx.Timeout(30.0), verify=certifi.where()) as client:
                                r = await client.get(full_img_url); r.raise_for_status()
                            with open(img_path, "wb") as f: f.write(r.content)
                            img_results.append({"url": full_img_url,
                                                "path": img_path.replace("\\", "/")})
                            print(f"[+] [POLICY TEST] 이미지 저장: {img_path}", flush=True)
                        except Exception as e:
...
async def execute_prism_scraping(req: PrismRequest):
    ...
    is_interrupted = app.state.stop_scraping or last_data_reached or detail_last_data_reached
    result_block   = _build_result(view_data, error_logs, is_interrupted)
    if (last_data_reached or detail_last_data_reached) \
            and result_block["status"] in ("SUCCESS", "PARTIAL"):
        result_block["message"] = "추가수집 완료 (last_data 기준점 도달)"
...
def _route_request(raw: UnifiedRequest):
    ...
    if raw.type == "prism":
        ...
        return PrismRequest(
            req_id=raw.req_id, type=raw.type, crw_id=raw.crw_id,
            file_dir=raw.file_dir,
            bbs_id=extra.get("bbs_id", "0"),
            param=PrismParam(**raw.param), item=raw.item,
...
async def crawl(raw: UnifiedRequest, background_tasks: BackgroundTasks):
    try:
        ...
        elif isinstance(req, PrismRequest):
            background_tasks.add_task(execute_prism_scraping,   req)
...
async def crawl_test(raw: UnifiedRequest):
    try:
        ...
        if isinstance(req, PrismRequest):
            return await execute_prism_scraping_test(req)
        if isinstance(req, PolicyRequest):
